...
class AddCinema extends Component {
	...
	onSubmitAction = async (type) => {
		const { getAllTheater, createTheater, updateTheater, removeCinemas } =
			this.props;
		const { _id, name, image, ticketPrice, city, seatsAvailable, seats } =
			this.state;
		const cinema = { name, ticketPrice, city, seatsAvailable, seats };
		let notification = {};
		type === 'create'
			? (notification = await createTheater(image, cinema))
			: type === 'update'
			? (notification = await updateTheater(image, cinema, _id))
			: (notification = await removeCinemas(_id));
		this.setState({ notification });
		if (notification && notification.status === 'success') getAllTheater();
	};
